...
The strategy development process employs a unique two-phase approach, with the first phase focused on discovering potential edge through extensive randomized testing before committing to detailed backtesting.

\subsubsection*{Initial Edge Discovery}
Instead of immediately running a complete backtest on a single strategy implementation, I wrote a custom python frame work which first tests whether a potential entry condition has any statistical merit \href{https://github.com/AJslashTracey/signal_search_strategy}{Signal Search framework}. This is accomplished by running 1000 different variations of the same base strategy. Each variation differs not in its entry condition, but in which specific entry signals it takes.

For example, if the base entry condition identifies 1000 potential trade entries over the historical data, each strategy variation will randomly sample different subsets of these entries (e.g., taking 20%, 80%, or 100% of all signals). This creates 1000 unique strategies that all use the same entry condition but trade different specific instances of it.
...
To further validate any potential edge, each of the 1000 variations is tested across different holding periods. This adds another dimension to the analysis - not only are we testing if the entry condition has merit, but also how long positions should be held to best capture any edge.
