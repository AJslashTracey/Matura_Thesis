...
This thesis explores whether I can develop an autonomous trading algorithm that can generate me a consistent profit. To that end my thesis will cover the entire development cycle of an algorithmic trading strategy. The process will include finding a trading strategy, backtesting the trading strategy,, applying the strategy to a live market, and evaultating the results.



The work is divided in three parts: Finding a strategy, Backtesting, and Live trading.

Finding a strategy can be done in multiple ways just by watching markets over longer time periods to get ideas, reading books about how other people were able to develope strategies and steal logic from them. In my case I will try to find a strategy by analysing the orderbook delta and identifying trends on the not stationary \footnote{Not stationary means not being still or fixed in one place; moving or changing rather than remaining constant.} bitcoin price. 
...
The project requires know how in three key areas: Statistics, Programming and understanding how the cryptocurrencies market operates. I need to apply stasticial rules to check if my idea is worthless or has potential to make me a profit if I automate the execution process. Programming is just needed to make the process of testing or applyinng my ideas easy on large datasets since I have to test on timeseries including up to 200'000 values. The algorithm will be written in Python and will run 24 hours a day seven days a week.
...
\subsubsection*{What is Bitcoin?}
Before explaining what Bitcoin is, we need to understand the fundamental problems with fiat money\footnote{Fiat money is a form of money that is not backed by a physical asset, but by the government or central bank that issues it.}. 
The core issue lies in the centralization, where goverments and central banks have complete control over the money supply. This control allows them to print money as needed increasing the money supply, oftean leading to inflation. A decrease in purchasing power devalues people's savings. Furthermore, money policy decisions are often influenced from political considerations over economic principles leading to poor economic outcomes. The entire system relies on the trust in the goverment controlling the money supply and their economical management abilities. 

Bitcoin offers a solution to these problems by being decentralized. i.e. no central authority controls it. The money supply is limited to 21 million Bitcoins. Anyone with internet access can use Bitcoin. Transactions are immutable and cannot be changed once confirmed. Bitcoin is borderless and can be sent anywhere in the world.

\subsubsection*{Investment demand}
...
\subsubsection*{Key Properties of Bitcoin}

Bitcoin has several unique properties that differentiate it from traditional currencies:

\begin{itemize}
    \item \textbf{Decentralized:} No central authority controls Bitcoin.
    \item \textbf{Limited Supply:} Only 21 million Bitcoins will ever exist.
    \item \textbf{Permissionless:} Anyone with internet access can use Bitcoin.
    \item \textbf{Immutable:} Transactions cannot be changed once confirmed.
    \item \textbf{Borderless:} Bitcoin can be sent anywhere in the world.
\end{itemize}
